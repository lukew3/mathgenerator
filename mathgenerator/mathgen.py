    ...
    if OnlyIntegerElementsInInvertedMatrix==True:
        ...
        while (isItOk==False):
            ...
            for i in range(0,SquareMatrixDimension):
                ...
                z[i]=1
                Mat.append(z)
            MaxAllowedMatrixElement=math.ceil(pow(MaxMatrixElement,1/(SquareMatrixDimension)))
            randomlist=random.sample(range(0,MaxAllowedMatrixElement+1),SquareMatrixDimension)
            
            for i in range(0,SquareMatrixDimension):
                if i==SquareMatrixDimension-1:
                    Mat[0]=[j+(k*randomlist[i]) for j,k in zip(Mat[0],Mat[i])]
                else:
                    Mat[i+1]=[j+(k*randomlist[i]) for j,k in zip(Mat[i+1],Mat[i])]
                
            
            for i in range(1,SquareMatrixDimension-1):
                Mat[i]=[sum(i) for i in zip(Mat[SquareMatrixDimension-1],Mat[i])]
        ...
        randomlist=random.sample(range(0,MaxMatrixElement+1),SquareMatrixDimension*SquareMatrixDimension)
        randomlist=list(set(randomlist)-set(plist))
        n_list=random.sample(randomlist,SquareMatrixDimension*(SquareMatrixDimension-1))
        Mat=list()
        for i in range(0,SquareMatrixDimension):
            z=list()
            z.append(plist[i])
...
invertmatrix=Generator("Inverse of a Matrix",60,"Inverse of a matrix A is","A^(-1)",MatrixInversion)
